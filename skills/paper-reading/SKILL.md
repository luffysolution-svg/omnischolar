...
4. Read the returned `publication.markdownPath`, its bibliographic frontmatter, `metadata.json`, and sibling assets with the host's local file and image capabilities. Parsed image filenames begin with the Zotero key. The complete parsed Markdown stays on disk.
